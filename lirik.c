...
void kembalikanAlat() {
    ...
    Alat alat, a;
    char uname[50];
    unsigned int id, jumlahPinjam = 0;
    int found = 0;

    printf("Masukkan ID alat yang ingin dikembalikan: ");
    scanf("%u", &id);

    // --- 1. Cari berapa jumlah yang dipinjam user pada ID itu ---
    ...
    rewind(pinjam);  // ulangi baca pinjam.txt dari awal

    // --- 2. Tambahkan kembali ke stok alat ---
    while (fscanf(alatFile, "%u, %49[^,], %49[^,], %49[^,], %u, %u\n",
        &alat.id, alat.nama, alat.merek, alat.model, &alat.tahun, &alat.jumlah) != EOF) {

        if (alat.id == id) {
            alat.jumlah += jumlahPinjam;  // FIX ★
        }
    ...
    // --- 3. Hapus baris pinjaman user tersebut ---
    while (fscanf(pinjam, "%[^,], %u, %49[^,], %49[^,], %49[^,], %u, %u\n",
        uname, &a.id, a.nama, a.merek, a.model, &a.tahun, &jumlahPinjam) != EOF) {
